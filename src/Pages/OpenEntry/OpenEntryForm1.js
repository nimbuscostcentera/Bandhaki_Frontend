...
//---------------------------------other states----------------------------------------
  ...
  const handleDetailChange = (index, e) => {
    ...
    if (name === "grossWeight" || name === "percentage") {
      const grossWeight =
        name === "grossWeight"
      ...
      if (grossWeight && percentage) {
        ...
        // If rate exists, calculate valuation
        const rate = Number.parseFloat(updatedRows[index].rate) || 0;
        if (rate) {
    ...
    if (name === "rate") {
      const netWeight = Number.parseFloat(updatedRows[index].netWeight) || 0;
  ...
  useEffect(() => {
    if (detailRows?.length > 0) {
      fetchGoldRate({ CompanyID: CompanyID,today:moment().format("YYYY-MM-DD") });
    }
  }, [detailRows?.length]);
  //setgoldrate
  useEffect(() => {
