...
function Index() {
  ...
  // };
  const handleReturn = (e) => {
    let setid = selectedId[0]?.Update_Mahajon_Voucher;
    if (setid) {
      toast.error("Allocated to a Mahajon");
      return;
    }
    setShowConfirmModal(true);
  };
  ...
  const ProceedHandler = () => {
    ...
    CloseModal();
    setFilteredData(arr);
    setFilter((prev) => ({ ...prev, HeaderID: Filter?.HeaderID }))
  };
  ...
  useEffect(() => {
    let list = [];
    if (isSearchFrAdjustSucc && Filter?.Adj_Cust_Type > 0) {
      console.log(
        isSearchFrAdjustSucc,
        entityType,
        Filter?.Adj_Cust_Type,
        SearchFrAdjustList,
        "find"
      );
    ...
    ClearStateSearchFrAdjust();
  }, [Filter?.Adj_Cust_Type, isSearchFrAdjustSucc, entityType]);

  useEffect(() => {
    if (
      isSelectedSearchListSucc &&
      Filter?.Adj_Cust_Type > 0 &&
      Filter?.HeaderID > 0
    ) {
  ...
  }, [isAllPaidLotSucc, errorAllPaidLots]);
  ...
  useEffect(() => {
    ...
    fetchSelectedSearchList({ ...Filter,Adj_cust_type:entityType, ...user });
  }, [Filter?.HeaderID]);
  //call search data for adjust
  useEffect(() => {
    console.log(user);
    if (
      Filter?.Adj_Cust_Type == -1 ||
      !Filter?.Adj_Cust_Type ||
      Filter?.Adj_Cust_Type == 0
    ) {
      return;
    ...
    fetchSearchFrAdjustMaster({
      Cust_Type: Filter?.Adj_Cust_Type,
      CompanyID,
      ...user,
    });
  }, [Filter?.Adj_Cust_Type]);
      ...
      <Row className="pt-2">
        ...
        <Col xl={1} lg={1} md={1} sm={1} xs={1}>
          <Button
            ...
            className="p-0 m-0"
            onClick={OpenModal}
            disabled={Filter?.HeaderID > 0 ? false : true}
          >
          ...
          <div className="table-box">
            {filteredData.length > 0 ? (
              <Table
                ...
                isLoading={isLoadingAllPaidLots}
              />
            ) : isLoadingAllPaidLots ? (
