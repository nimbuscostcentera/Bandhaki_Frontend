...
import useEditMahajon from "../../store/UpdateStore/useEditMahajon";
...
function MahajonDetails({ isDisable, setIsDisable, setTextDetail, search }) {
  ...
  const {
    ...
    ClearMahajonDelete,
    MahajonDeleteErr,
    isMahajonDeleteLoading,
    MahajonDeleteMsg,
  } = useMahajonDelete();
  const {
    MahajonEditError,
    isMahajonEditLoading,
    MahajonEditSuccess,
    EditMahajonFunc,
  ...
  const ActionFunc = (tabindex) => {
    ...
    setIsDisable(true);
    setEditedData({
      MahajanID: filteredData[tabindex]?.ID,
  ...
  const handleSearch = (e) => {
    ...
    const filtered = MahajonList.filter((order) =>
  ...
  //toaster
  useEffect(() => {
    if (isMahajonEditLoading && !MahajonEditSuccess && !MahajonEditError) {
      toast.play("pleaes wait...", {
        position: "top-right",
        autoClose: 3000,
      });
    } else if (MahajonEditSuccess && !isMahajonEditLoading && !MahajonEditError) {
      toast.success("Mahajon Edited Successfully", {
        position: "top-right",
        autoClose: 3000,
      });
      setEditedData({
      ...
      setIsDisable(false);
    } else if (MahajonEditError && !isMahajonEditLoading && !MahajonEditSuccess) {
      toast.error(MahajonEditError, {
        position: "top-right",
        autoClose: 3000,
      });
    }
    ClearStateEditMahajon();
  }, [isMahajonEditLoading, MahajonEditSuccess, MahajonEditError]);

  useEffect(() => {
    if (MahajonDeleteMsg) {
      toast.success(MahajonDeleteMsg, {
        position: "top-right",
        autoClose: 3000,
      });
    }
    if (MahajonDeleteErr) {
      toast.error(MahajonDeleteErr, {
        position: "top-right",
        autoClose: 3000,
      });
    }
    ClearMahajonDelete();
  }, [MahajonDeleteErr, isMahajonDeleteLoading, MahajonDeleteMsg]);
  ...
  useEffect(() => {
    fetchMahajonData({ CompanyID: CompanyID });
  }, [MahajonRegSuccess, MahajonEditSuccess, MahajonDeleteMsg]);
  console.log(
    "MahajonList",
    filteredData,
    isLoadingMahajon,
  ...
  useEffect(() => {
    if (MahajonList?.length >= 0) {
      let updatedData = MahajonList?.map((item) => {
